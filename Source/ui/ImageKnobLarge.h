...
public:
#if USE_PARAM
    ImageKnobLarge(ConcurrentParameterSet &parameters,
                   const ParameterString &name,
                   const ResourceCache *resources) :
    ImageKnob(parameters, name, resources, "large_knob") {}

#else
    ImageKnobLarge(const ResourceCache *resources)
     : ImageKnob(resources, "large_knob") { }

#endif

    virtual ~ImageKnobLarge() {}
    
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(ImageKnobLarge);
};
