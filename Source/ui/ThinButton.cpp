...
void ThinButton::paint(Graphics &g) {
    ...
    ThinButton::ThinButton(const ResourceCache *resources, const String &imageName)
     : ImageButton(juce::String()),
       ComponentWithCachedGraphics(resources, imageName),
       gravity(kGravityDefault)
    {
        juce::ImageButton::setImages(false, false, true,
                                     imageStates->normal,     1.0f, juce::Colour(0x0),
                                     juce::Image(),          1.0f, juce::Colour(0x0),
                                     imageStates->alternate, 1.0, juce::Colour(0x0));
    }

    void ThinButton::paint(Graphics &g)
    {
        const juce::Image buttonStateImage = getImageForButtonState();
        int buttonY = 0;
        const int buttonHeight = buttonStateImage.getHeight();
        const int buttonWidth = buttonStateImage.getWidth();
