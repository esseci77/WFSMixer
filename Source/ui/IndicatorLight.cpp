...
namespace teragon {
#ifdef USE_PARAM
IndicatorLight::IndicatorLight(ConcurrentParameterSet &parameters,
...
void IndicatorLight::onParameterUpdated(const Parameter* parameter) {
    ...
    IndicatorLight::IndicatorLight(const ResourceCache *resources)
    : Component(juce::String()),
      ComponentWithCachedGraphics(resources, "indicator_light"),
      lightOn(false),
      pulse(false)
    {
        enabledImage = imageStates->normal;
...
void IndicatorLight::setLightOn(bool onOff, bool pulseOn)
{
    if (this->lightOn != onOff)
    {
        this->lightOn = onOff;
        this->enabledOpacity = onOff ? 0.0f : 1.0f;
        this->stepRate = (onOff ? 1.0f : -1.0f) * kFadeDurationStepRate;
        this->pulse = pulseOn;
