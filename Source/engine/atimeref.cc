...
int32_t Atimetab::_ff [6] = { FR0, FR1, FR2, FR3, FR4, FR5 };
...
void Atimetx::process (int32_t nframes, float *out)
{
    int32_t     i, j;
    float         v;
...
void Atimerx::process (int32_t nframes, float *inp)
{
    int32_t  i, j;
    float       v;
    Atimefreq   *F;
...
int32_t Atimerx::resolve (void)
{
    ...
    for (i = 1; i < 6; i++)
    {
        ...
        a = DIV1 * (a - floorf (a));
        j = (int32_t)(floorf (a + 0.5f));
        if (fabsf (a - j) > 0.3f) return -3;
        p += m * (j & (DIV1 - 1));
        m *= DIV1;
    }  
    if (p < 0) p += FR0;
    return (int32_t)(DIV0 * p + 0.5f) & MMASK;
}
